...
void MeshBuilder::preallocateMeshElements(const MeshData& data, Mesh& mesh)
{
...
void MeshBuilder::indexVertices(Mesh& mesh)
{
    int index = 0;
...
void MeshBuilder::checkNonManifoldVertices(const Mesh& mesh)
{
    std::map<VertexCIter, int> vertexFaceMap;
    
    for (FaceCIter f = mesh.faces.begin(); f != mesh.faces.end(); f++) {
        HalfEdgeCIter he = f->he;
        do {
            vertexFaceMap[he->vertex] ++;
            he = he->next;
